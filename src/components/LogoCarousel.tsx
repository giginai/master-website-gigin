      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div ref={scrollerRef} className="scroller overflow-hidden [mask-image:linear-gradient(to_right,transparent,white_20%,white_80%,transparent)]">
          <div className="scroller-inner flex gap-16 animate-scroll hover:[animation-play-state:paused]">
            {logos.map(logo => (
              <div key={logo.id} className="flex items-center justify-center min-w-[280px] h-20 p-6 flex-shrink-0 px-0 py-0">
                <img 
                  src={logo.src} 
                  alt={logo.name} 
                  className="max-w-full max-h-full filter hover:filter-none transition-all duration-300 opacity-80 hover:opacity-100 object-contain" 
                />
              </div>
            ))}
          </div>
