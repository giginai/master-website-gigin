      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative mb-16">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-4">
                    {differentiators.slice(slideIndex * 2, slideIndex * 2 + 2).map((item, index) => <div key={slideIndex * 2 + index} className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 group hover:-translate-y-1 py-[24px]">
                        ...
                        <div className="space-y-2">
                          {item.features.slice(0, 3).map((feature, featureIndex) => <div key={featureIndex} className="flex items-center text-sm text-gray-600">
          ...
          <div className="flex justify-center space-x-2 mt-8">
            {Array.from({
            length: Math.ceil(differentiators.length / 2)
          }, (_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-pink-500' : 'bg-gray-300'}`} />)}
          </div>
        </div>
