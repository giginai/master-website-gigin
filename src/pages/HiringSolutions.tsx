        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="relative max-w-3xl mx-auto">
            <div className="relative overflow-hidden rounded-t-3xl group">
              <img src="/lovable-uploads/31c4796a-7366-4dc3-8833-cc1ae604f052.png" alt="Gigin Hiring Platform Dashboard" className="w-full h-auto object-contain transition-transform duration-700 ease-out group-hover:scale-105" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="job-marketplace" className="relative z-10 py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            {jobMarketFeatures.map((feature, index) => <Card key={index} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-gray-100 group">
          ...
          <div className="text-center">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
              <div>
                <div className="text-4xl font-bold mb-2 text-pink-500">3M+</div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 border-b border-gray-200">
              {atsFeatures.map((feature, index) => <button key={index} onClick={() => handleTabChange(index)} className={`relative p-4 text-xs lg:text-sm font-medium transition-all duration-300 ${currentTab === index ? 'bg-pink-500 text-white' : 'text-gray-600 hover:text-pink-600 hover:bg-pink-50'}`}>
                  {feature.title}
                  {currentTab === index && <div className="absolute bottom-0 left-0 w-full h-1 bg-pink-400">
                      <div className="bg-white h-full transition-all duration-100 ease-linear" style={{
                  width: `${progress}%`
                }} />
                    </div>}
                </button>)}
              ...
              <div className="grid lg:grid-cols-2 gap-8 items-center">
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {atsFeatures[currentTab].trustIndicators.map((indicator, idx) => <div key={idx} className="flex items-center bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">
                ...
                <div className="relative">
                  <img src={atsFeatures[currentTab].image.startsWith('http') ? atsFeatures[currentTab].image : `https://images.unsplash.com/${atsFeatures[currentTab].image}?auto=format&fit=crop&w=800&q=80`} alt={atsFeatures[currentTab].title} className="w-full h-80 object-cover rounded-2xl shadow-lg" />
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="guaranteed-hiring" className="relative z-10 py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {bulkHiringCards.map((card, index) => <Card key={index} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-gray-100 group overflow-hidden">
                <div className="relative h-48">
                  <img src={card.image.startsWith('http') ? card.image : `https://images.unsplash.com/${card.image}?auto=format&fit=crop&w=800&q=80`} alt={card.title} className="w-full h-full object-cover" />
          ...
          <div className="relative max-w-4xl mx-auto px-16">
            <div className="overflow-hidden rounded-3xl">
              ...
              transform: `translateX(-${currentSlide * 100}%)`
                ...
                {aiAgentCards.map((card, index) => <div key={index} className="w-full flex-shrink-0">
                      ...
                      <div className="grid lg:grid-cols-2 gap-0 min-h-[450px]">
                        <div className="p-6 lg:p-8 flex flex-col justify-center">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {card.trustMarkers.map((marker, idx) => <div key={idx} className="flex items-center bg-blue-100 text-blue-800 px-3 py-1 rounded-lg text-xs lg:text-sm font-medium">
            ...
            {/* Carousel Controls */}
            <button onClick={prevSlide} className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 group z-10">
              <ChevronLeft className="w-6 h-6 text-gray-600 group-hover:text-pink-600" />
            </button>
            <button onClick={nextSlide} className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 group z-10">
              <ChevronRight className="w-6 h-6 text-gray-600 group-hover:text-pink-600" />
            </button>

            {/* Carousel Indicators */}
            <div className="flex justify-center space-x-2 mt-8">
              {aiAgentCards.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-pink-500' : 'bg-gray-300'}`} />)}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="relative max-w-3xl mx-auto">
            <div className="overflow-hidden rounded-t-3xl">
              <img src="/lovable-uploads/31c4796a-7366-4dc3-8833-cc1ae604f052.png" alt="Gigin Platform Dashboard" className="w-full h-auto object-contain" />
            </div>
          </div>
        </div>
      </section>
